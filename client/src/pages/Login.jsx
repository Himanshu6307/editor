...
function Login() {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true)
    try {
      const response = await AxiosInstance.post("http://localhost:4000/api/v1/auth/login", {email, password })
    ...
    <div className="flex justify-center items-center min-h-screen bg-black">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        ...
        >
         ...
         <p onClick={()=>{navigate("/signup")}} className='text-blue-600 underline mt-2 cursor-pointer'>Create Account</p>
      </div>
    </div>
  )
}
